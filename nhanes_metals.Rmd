...
Of the 9,971 participants surveyed in 2015-2016, `r print(nrow(nh1516_sub))` were eligible to have blood metals measured.
...
nh1516_sub %>%
  count(is.na(lbxbpb))
# 10.88% still missing data.... seems extremely high, should probably check to see whether missingness is at random or not 
...
From our eligible subset, about 11% had missing values for blood metals Pb, Cd, Mn, and total Hg. Thus, after removing participants with missing values, we are left with `r print(nrow(nh1516_sub))` for analysis.

## Inorganic, Ethyl, and Methyl Mercury: Subset Data Cleaning and Evaluation

### Missing Data

```{r create separate dataset for speciated mercury}
# assess missingness 
...
Because there were additional participants missing values of mercury (inorganic, ethyl, and methyl), we created a separate dataset, `hg1516`, for blood mercury. After retaining only complete cases, we are left with `r print(nrow(hg1516))` participants for mercury analysis. 

## Urinary Mercury: Subset Data Cleaning and Evaluation

```{r create urinary mercury dataframe}
...
```{r remove NAs and 0 weights for urinary Hg }
...
Of the 9,971 participants surveyed in 2015-2016, `r print(nrow(uhg1516_sub))` were eligible to have urine mercury measured.

```{r drop missing mercury and creatinine}
...
Furthermore, out of those eligible, about 4.6% were missing values for urinary mercury. After dropping those with values missing for urinary mercury and/or creatinine (needed for adjustment), we have `r print(nrow(uhg1516_sub))` participants remaining for analysis. 
...
```{r remove NAs and 0 weights for urinary arsenic}
...
Of the 9,971 participants surveyed in 2015-2016, `r print(nrow(uas1516_sub))` were eligible to have urine arsenic measured.

```{r drop missing urinary arsenic and DMA}
...
Furthermore, out of those eligible, 5.4% were missing values for total arsenic and DMA. After dropping those with values missing for urinary arsenic and/or creatinine (needed for adjustment), we have `r print(nrow(uas1516_sub))` participants remaining for analysis. 

### Total N's

There were `r print(nrow(nh1516))` participants surveyed in the NHANES 2015-2016 cycle.

In our data subsets, there are:

* `r print(nrow(nh1516_sub))` participants with Pb, Cd, Mn, and total Hg measured in blood,

* `r print(nrow(hg1516_sub))` participants with inorganic, ethyl, and methyl Hg measured in blood,

* `r print(nrow(uhg1516_sub))` participants Hg measured in urine,

* `r print(nrow(uas1516_sub))` participants with total and speciated arsenic measured in urine.
...
## Survey-Weighted Data Distribution
...
```{r graph these means for sex and age group, Pb}
...
As we can see, there was much higher concentrations of organic Hg than inorganic Hg for Hg measured in blood.
...
```{r urine mercury data cleaning}

# Merge data to create one dataframe per year/NHANES cycle
uhg_1516 <- merge(demo1516, uhg1516, by = "SEQN", all = TRUE)
...
# Exclude those who have WTSA2YR as "NA"
uhg_1516 <-
  uhg_1516 %>% 
  drop_na(wtsa2yr, urxuhg)

```

Next, we set the survey design again to look at the mean urinary mercury across males and females.

```{r urine mercury survey weighting}

uhg.svy <- svydesign(ids = ~sdmvpsu, # cluster IDs
                        strata = ~sdmvstra, # strata IDs
                        weights = ~wtsa2yr, # individual weights, subsample descriptions and weights are found in the specific datafiles downloaded from NHANES website
                        nest = TRUE,
                        data = uhg_1516) # call the dataframe 

summary(uhg.svy) 

```

```{r urine mercury statistics and graphing}
...
```{r download arsenic data and clean}
...
arsenic.svy <- svydesign(ids = ~sdmvpsu, # cluster IDs
                        strata = ~sdmvstra, # strata IDs
                        weights = ~wtsa2yr, # individual weights, subsample descriptions and weights are found in the specific datafiles downloaded from NHANES website
                        nest = TRUE,
                        data = arsenic_1516) # call the dataframe 

summary(arsenic.svy) 
```

```{r arsenic descriptive stats}
