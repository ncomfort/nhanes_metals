...
svymean(~log(lbxbpb)+log10(lbxbcd)+log(lbxbmn), # continuous variable(s)
        blood.svy) # the ”svydesign” object

# put results into a table
  svymean(~log(lbxbpb)+log10(lbxbcd)+log(lbxbmn),
               blood.svy) %>% 
  kable() %>% 
  kable_styling(bootstrap_options = c("striped", "hover"),
                full_width = F) 
# note that lbxbpb is in the units ug/dL whereas the others are in ug/L
# exponentiate these values to get the geometric means
...
Find the geometric mean for males and females and determine if the difference is statistically significant. 

```{r blood Pb, Cd, Mn by sex}
...
```{r speciated blood Hg overall pop}
# Count number of males and females in this dataset
...
Find the mean levels of Pb, Cd, Hg (inorganic and organic), and Mn in the US population (accounting for survey weights and design). 


```{r descriptive stats using survey functions, eval=FALSE, include=FALSE}
