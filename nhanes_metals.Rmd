...
```{r blood metals check distributions}
...
```{r speciated blood Hg check distributions}
...
```{r speciated blood Hg check impact of influential outliers}


# plot the survey weight against the variable to determine which extreme values are outliers
hg1516_sub %>% 
  ggplot(aes(x = log(lbxihg), y = wtsh2yr)) +
  geom_point()

hg1516_sub %>% 
  ggplot(aes(x = log(lbxbge), y = wtsh2yr)) +
  geom_point()
...
```{r urinary Hg check distributions}
...
```{r urinary Hg describe impact of influential outliers}
...
```{r urinary As check distributions}
...
```{r urinary As describe impact of influential outliers}
...
Below are the geometric mean levels (and 95% CIs) of blood Pb, Cd, Mn, and Hg (total and meHg), as well as urinary Hg levels and total As and DMA in the US population (accounting for survey weights and design):
...
pb_sex_means %>% 
  ggplot(aes(x = Sex, 
             ...
             fill = Sex)) +
  geom_point() +
  geom_errorbar(aes(ymin = lower, # change for different metalas
                    ymax = upper),
                    width = 0.2,
                    position = position_dodge(0.9)) +
  labs(title="Levels of Blood Pb in Males vs Females", # edit labels for diff graphs
       x = "Sex",
       y = "Blood Pb (ug/dL)",
...
Is the log(Cd) distribution normal enough for t-test? 
...
Oddly, the levels of inorganic, ethyl, and methyl mercury in the NHANES dataset ` r IHGEM_I.XPT` do not sum to the value of total mercury in the `r PBCD_I.XPT` NHANES dataset. Thus, I am only going to analyze methyl mercury and total mercury. 

**Is the log transformation ok for total Hg and meHg?** 
...
hg_sex_means %>% 
  ...
  geom_errorbar(aes(color = Type,
                    ...
                    ymax = upper),
                    width = 0.2) +
  labs(title = "Levels of Blood meHg in Males vs Females", # edit labels for diff graphs
       x = "Sex",
       y = "Blood meHg (ug/L)",
       caption = "NHANES 2015/2016 data") 
